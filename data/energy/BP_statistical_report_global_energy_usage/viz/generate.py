...
# Load the data
data = pd.read_csv('../processed/bp_consumption_by_type.csv')
data['year'] = pd.to_datetime(data['year'], format='%Y')
data['consumption_GW'] = data['consumption_TW'].values * 1E3
data['consumption'] = anthro.io.numeric_formatter(
    data['consumption_TW'] * 1E12, sci=True, unit='W')
for g, d in data.groupby(['type']):
    ...
    chart = alt.Chart(d).encode(
        ...
        y=alt.Y(field='consumption_GW',  type='quantitative',
                title=f'power consumption [{title}]'),
