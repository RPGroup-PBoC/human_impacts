...
observed = pd.read_csv('../processed/observed_GMSL_from_1900.csv')
...
chart = alt.Chart(observed).encode(
        ...
        tooltip=[alt.Tooltip(field='year', type='temporal', format='%Y', title='year'),
                 alt.Tooltip(field='observed_GMSL_mean', type='quantitative', title='mean [mm]')]
...
contrib = pd.read_csv('../processed/contributor_GMSL_from_1900.csv')
...
for g, d in contrib.groupby('source'):
    ...
    chart = alt.Chart(observed).encode(
            ...
            tooltip=[alt.Tooltip(field='year', type='temporal', format='%Y', title='year'),
                     alt.Tooltip(field='mean', type='quantitative', title='mean [mm]')]
        ).properties(width='container', height=300)
