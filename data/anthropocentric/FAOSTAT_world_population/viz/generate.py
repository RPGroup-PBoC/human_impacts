...
agg_data = data.groupby(['year']).apply(lambda x: x[x['category'] == 'urban']
                                        ['population'].values[0] / x['population'].sum()).reset_index()
agg_data.columns = ['year', 'urban_fraction_pct']
agg_data['urban_fraction_pct'] *= 100
agg_data['urban_population'] = anthro.io.numeric_formatter(
    data[data['category'] == 'urban']['population'].values, sci=False)
agg_data['total_population'] = anthro.io.numeric_formatter(
    data.groupby(['year'])['population'].sum().values, sci=False)
agg_data
# %%
# Generate the plot.
chart = alt.Chart(agg_data).encode(
    ...
    tooltip=[alt.Tooltip(field='year', type='temporal', timeUnit='year', title='year'),
             alt.Tooltip(field='urban_population',
                         type='nominal', title='urban population'),
...
data['population'] = data['population_MHd'] * 1E6
data['year'] = pd.to_datetime(data['year'].values, format='%Y')
data['population_Bhd'] = data['population'] * 1E-9
data['population'] = anthro.io.numeric_formatter(
    data['population'].values, sci=False)

chart = alt.Chart(data).encode(
    x=alt.X(field='year', type='temporal', timeUnit='year', title='year'),
    y=alt.Y(field='population_Bhd', type='quantitative',
