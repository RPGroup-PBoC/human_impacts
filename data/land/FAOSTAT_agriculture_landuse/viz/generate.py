...
data['land_area'] = anthro.io.numeric_formatter(data['land_area_Mha'].values * 1E6, 
                                                sci=False, unit='Ha')
data['land_area_Bha'] = data['land_area_Mha'].values / 1E3
...
data = data.groupby('year').apply(lambda x: x[x['usage_type']=='pasture']['area_km2'].values[0] / x['area_km2'].sum()).reset_index()
data.columns = ['year', 'fraction_pct']
data['fraction_pct'] *= 100
data['area_ha'] = anthro.io.numeric_formatter(area_ha.values, unit='Ha', sci=False)
...
chart = alt.Chart(data).encode(
            ...
            tooltip=[alt.Tooltip(field='year', type='temporal', format='%Y', title='year'),
                     alt.Tooltip(field='fraction_pct', type='quantitative', format='0.0f', title='fraction [%]'),
